...
本项目作为一个学习项目，只从功能上进行实现，对性能不做太高的要求，因此在吞吐量方面还有待提升。关于性能，可以从以下几个方面进行提升：

1. 序列化：使用压缩比更高结构更紧凑的二进制序列化协议，本项目使用了Json+protobuff，有进一步优化空间
2. 引入NIO：利用NIO的非阻塞特性，可以显著提高系统的并发处理能力，尤其是在处理大量轻量级连接的情况下。
3. 零拷贝：考虑使用sendfile或mmap等技术实现零拷贝，以减少数据从用户态到内核态的复制次数，从而提高I/O效率。
4. 异步刷盘+批次写入：本项目都使用的同步刷盘+单条消息写入，如果换成异步刷盘+批次写入的话，性能会有大幅度提升，但是需要处理好消息丢失的问题
5. 批次读取：一次pull多条数据到本地进行消费，避免多次请求建立连接
6. 使用更底层的通讯协议：考虑使用更底层的协议：如TCP或UDP，这些协议相比HTTP具有更低的开销，更适合于高性能的通信场景
7. 对象的精简、模块与对象的共用：需要进行针对性的优化
8. 数据结构和算法（数据量大的时候，低效算法耗时指数级上升）：这一块有少量优化空间 : )
